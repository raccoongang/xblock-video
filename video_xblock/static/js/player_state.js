...
var player_state_obj = window.player_state_obj;
var player_state = {
...
var setInitialState = function(player, state) {
    ...
    if (playbackProgress){
        ...
        if (playbackProgress[window.video_player_id]) {
            stateCurrentTime = playbackProgress[window.video_player_id];
        }
    }
...
var saveProgressToLocalStore = function saveProgressToLocalStore() {
    ...
    playbackProgress = JSON.parse(playbackProgress);
    playbackProgress[window.video_player_id] = player.ended() ? 0 : player.currentTime();
